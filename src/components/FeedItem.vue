...
<template>
  ...
  <template v-if="post.attachment.length">
    <img v-for="image in post.attachment" :key="image.id" :src="image.get_image" class="w-full mb-4 rounded-xl" />
  </template>
  <p>{{ post.body }}</p>
  <div class="my-6 flex justify-between">
    <div class="flex space-x-6 items-center">
      ...
      <div class="flex items-center space-x-2">
        ...
        <RouterLink :to="{ name: 'post', params: { id: post.id } }" class="text-gray-500 text-xs">{{ post.comments_count }} Comments</RouterLink>
      </div>
      <div class="flex items-center space-x-2 text-gray-500 text-xs" v-if="post.is_private">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
  ...
  <div v-if="show">
    <div class="flex space-x-6 items-center justify-end">
      ...
      <div class="flex items-center space-x-2" @click="report">
